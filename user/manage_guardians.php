...
if (isset($_GET['delete_id'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM guardians WHERE id = ? AND user_id = ?");
...
if (isset($_POST['edit_guardian'])) {
    ...
    $stmt = $conn->prepare("UPDATE guardians SET guardian_name = ?, guardian_phone = ? WHERE id = ? AND user_id = ?");
    $stmt->bind_param("ssii", $guardian_name, $guardian_phone, $guardian_id, $user_id);
